        ...
        for side in left right; do
            for cortical in caudate accumbens putamen; do # Executive Motivational Motor; do
                for exclusion in Exclusion No_exclusion; do
                    ...
                    mkdir "$DIR/Tractography_${group}/$name/$exclusion/accumbens/$side"  #folder that will contain probtrackx output of each side of the brain
...
# --avoid		Reject pathways passing through locations given by this mask
                        ...
                        /usr/share/fsl/5.0/bin/probtrackx2 -x $DIR/$group/$name/Cortical_masks/accumbens/FA_image_${side}_normalized.nii.gz -l --onewaycondition -c 0.2 -S 2000 --steplength=0.5 -P 5000 --fibthresh=0.01 --distthresh=0.0 --sampvox=0.0 --avoid="$DIR/$group/$name/Exclusion_masks/exclusion_mask_total_accumbens_${side}.nii.gz" --forcedir --opd -s $bedpost_dir/merged -m $bedpost_dir/nodif_brain_mask --dir=$DIR/Tractography_${group}/$name/$exclusion/accumbens/$side/seed1 --stop=$DIR/$group/$name/accumbens_masks/FA_transformation/smoothed_${side}_accumbens.nii.gz 


                        /usr/share/fsl/5.0/bin/probtrackx2 -x $DIR/$group/$name/accumbens_masks/FA_transformation/smoothed_${side}_accumbens.nii.gz -l --onewaycondition -c 0.2 -S 2000 --steplength=0.5 -P 5000 --fibthresh=0.01 --distthresh=0.0 --sampvox=0.0 --avoid="$DIR/$group/$name/Exclusion_masks/exclusion_mask_total_accumbens_${side}.nii.gz" --forcedir --opd -s $bedpost_dir/merged -m $bedpost_dir/nodif_brain_mask --dir=$DIR/Tractography_${group}/$name/$exclusion/accumbens/$side/seed2 --waypoints=$DIR/$group/$name/Cortical_masks/accumbens/FA_image_${side}_normalized.nii.gz --waycond=AND
                        ...
                        /usr/share/fsl/5.0/bin/probtrackx2 -x $DIR/$group/$name/Cortical_masks/accumbens/FA_image_${side}_normalized.nii.gz -l --onewaycondition -c 0.2 -S 2000 --steplength=0.5 -P 5000 --fibthresh=0.01 --distthresh=0.0 --sampvox=0.0 --avoid="$DIR/$group/$name/Exclusion_masks/exclusion_mask_CSF_middline_accumbens_${side}.nii.gz" --forcedir --opd -s $bedpost_dir/merged -m $bedpost_dir/nodif_brain_mask --dir=$DIR/Tractography_${group}/$name/$exclusion/accumbens/$side/seed1 --stop=$DIR/$group/$name/accumbens_masks/FA_transformation/smoothed_${side}_accumbens.nii.gz 
